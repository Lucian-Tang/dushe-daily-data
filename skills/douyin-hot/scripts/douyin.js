...
function printJson(hotList) {
  console.log(JSON.stringify(hotList, null, 2));
}
...
async function main() {
  ...
  if (command === 'hot') {
    ...
    console.log('  node scripts/douyin.js json [数量]  # JSON输出');
    console.log('');
    console.log('示例:');
    console.log('  node scripts/douyin.js hot      # 获取热榜（默认50条）');
    console.log('  node scripts/douyin.js hot 20   # 获取热榜前20条');
    console.log('  node scripts/douyin.js json 10 # JSON输出前10条');
    process.exit(1);
  }
}
