#!/usr/bin/env python3
"""fetch_startup.py - 科技创业日报信源抓取 v2 (with retry + fallback)"""
...
def write_alert(level: str, message: str):
    ...
    # stdout — printed for cron/pipe capture
...
async def main():
    ...
    logger.info(f"🚀 开始抓取 startup 信源 ({len(SOURCES)} 个): {', '.join(source_names)}")

    # ── Round 1: fetch all sources in parallel ──
    async with httpx.AsyncClient(
        headers={"User-Agent": "StartupDailyBot/2.0"},
    ...
    if failed_sources and total_before_retry == 0:
        ...
        async with httpx.AsyncClient(
            headers={"User-Agent": "StartupDailyBot/2.0 (retry)"},
            follow_redirects=True,
            timeout=30
        ) as session:
    ...
    if len(unique) == 0:
        success = fallback_to_yesterday("raw_startup", date)
        if not success:
