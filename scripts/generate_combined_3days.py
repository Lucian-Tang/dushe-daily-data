...
def merge_and_deduplicate(items_by_day, section=""):
    ...
    for i, (date_str, items) in enumerate(items_by_day):
        ...
        for item in items:
            ...
            # Treat # as empty url (placeholder, not a real dedup key)
            url = (raw_url if raw_url and raw_url != '#' else '') or item.get('uid', '') or item.get('title', '')
...
def main():
    ...
    # 注入敏感词 blocklist 到 index.json（供客户端读取）
