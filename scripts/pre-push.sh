...
while read local_ref local_sha remote_ref remote_sha; do
    ...
    for f in $changed_files; do
        ...
        if [[ "$fname" =~ ^[a-z]+_daily_([0-9]{8})\.json$ ]]; then
            ...
            # ---- Check 3: 今日数据防清空检查 ----
            # 如果远程已有数据 > 0 条，本地提交为 0 条 → 阻止（避免误写空数组覆盖）
            if [ "$file_date" -eq "$TODAY" ] && [ -n "$remote_sha" ] && [ "$remote_sha" != "0000000000000000000000000000000000000000" ]; then
                old_items=$(git show "$remote_sha:$f" 2>/dev/null | python3 -c "import json,sys; d=json.load(sys.stdin); print(len(d))" 2>/dev/null)
                new_items=$(python3 -c "import json,sys; d=json.load(open('$WORKSPACE/$f')); print(len(d))" 2>/dev/null)
                if [ -n "$old_items" ] && [ "$old_items" -gt 0 ] && [ -n "$new_items" ] && [ "$new_items" -eq 0 ]; then
                    echo ""
                    echo "╔══════════════════════════════════════════════╗"
                    echo "║  🔴 今日数据防清空: $fname"
                    echo "║     远程现有: ${old_items} 条"
                    echo "║     本地提交: 0 条"
                    echo "║                                              "
                    echo "║  数据不会凭空消失，如果确认要清空请：        "
                    echo "║    git commit --allow-empty -m 'force reset' "
