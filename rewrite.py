...
entries = [
  {
    ...
    "quote": "监管的脚步永远追不上技术的狂奔，但刹车片总得先装上。",
  ...
  },
  {
    ...
    "quote": "上下文窗口从一万到两百万，AI的记忆力比人类开会时的记性好多了。",
  ...
  },
  {
    ...
    "content": "在遭到用户强烈反对后，微软在推进Teams会议AI功能部署方面做出让步。此前微软计划引入Facilitator协作助手、Intelligent Recap智能回顾以及Copilot功能，其中Recap集中展示最近30天会议录制与AI生成摘要，Facilitator可分析对话内容、识别未解答问题。微软澄清用户可根据需求在会议中途启用或禁用相关功能。企业IT管理员可定制租户策略控制Meeting AI的可见性。",
    "quote": "用户被AI功能支配的恐惧，微软终于听见了。",
  ...
  },
  {
    ...
    "quote": "当大家都在追逐GPU算力时，真正的瓶颈其实藏在数据传输的路上。",
  ...
  },
  {
    ...
    "quote": "一边教育AI要有道德，一边教员工如何突破道德底线。",
  ...
  },
  {
    ...
    "content": "研究机构SemiAnalysis数据显示，由于PCB中介板制造工艺难度过高，英伟达下一代AI机架系统Kyber可能推迟至2028年推出。这种核心电路板需通过复杂多层印刷连接系统内各模块。近年来英伟达坚持每年推出新一代AI产品，但随着系统规模越来越大，制造难度不断提升，高速更新节奏已开始受制造能力约束。报道称英伟达曾测试两套备用方案但最终放弃，原因是云服务商觉得设计过于奇怪且运营成本过高。",
  ...
  },
  {
    ...
    "content": "复旦大学数据挖掘技术课程结束了一场特殊期末考试。学生用自己设计的10道题去考三个不同水平的AI模型，AI答错越多学生得分越高。三个模型分三个难度梯度：DeepSeek V4-Flash答错一题+1.5分、MiniMax M2.7答错+2分、Claude Sonnet 4.6答错+3分。全班51份试卷中4人让某个AI模型整张卷得0分，但最强的Claude未被任何学生完全考倒。教授表示传统出题方式在AI时代已失效。",
...
for i, e in enumerate(data):
    assert len(e['content']) >= 60, f'Entry {i} content too short: {len(e["content"])}'
    assert 30 <= len(e['quote']) <= 80, f'Entry {i} quote wrong length: {len(e["quote"])}'
    assert e['uid'].startswith('ai_'), f'Entry {i} uid wrong'
    assert '(' not in e['source'] and ')' not in e['source'], f'Entry {i} source has parens'
    assert e['url'].startswith('https://'), f'Entry {i} url not https'
...
uids = [e['uid'] for e in data]
assert len(uids) == len(set(uids)), 'Duplicate UIDs!'

dates = sorted(set(e['published'] for e in data))
print(f'Dates: {dates}')
